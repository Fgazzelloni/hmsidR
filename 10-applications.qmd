...
In this example we have a look at hoe to visualize the results of a decision tree model for predicting Ischemic Stroke.
...
Set up the recipe for the data with the `recipe()` function from the `tidymodels` package. We will use the `step_corr()` function to remove highly correlated predictors, `step_center()` and `step_scale()` to standardize the predictors, `step_YeoJohnson()` to transform the predictors, and `step_zv()` to remove zero variance predictors.
...
Finally, we will fit the model with the `fit()` function from the `tidymodels` package and visualize the results with the `rpart.plot` package.
```{r}
is_wfl <- workflow() %>%
