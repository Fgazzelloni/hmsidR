...
It starts with the `ggplot()` function, which initializes the plot. The
...
variables, color, shape, and size.
...
and y coordinates, color, shape, and size. The `aes()` function is the
...
themes to customize the appearance of the plot. By combining these
elements, you can create visually appealing and informative
visualizations that effectively communicate your data.

It allows the user to create complex visualizations by combining simple
building blocks, with each layer of the plot allowing for an aesthetic
mapping of data to visual properties, such as color, fill, shape, and
size, making it easy to create a wide range of visualizations, or even
...
visualizations in R, such as `{plotly}`, `{ggplotly}`, `{leaflet}`,
...
engaging and dynamic data visualizations.
...
![Basic Data Viz
Rules](images/09_gt_table.png){#fig-dvrules
fig-align="center" fig-alt="Basic Data Viz
Rules"}
...
documentation and explore other options for customizing colors using the
`scale_<..>_<..>()` functions.
...
In addition to colors, patterns can be used to differentiate between the
groups in the plot. The `{ggpattern}` package provides a variety of
...
#| fig-cap: "Customized Lineplots and Log Scale Transformation"
#| fig-subcap: 
#|  - "Basic Lineplot"
#|  - "Customized Lineplot"
#|  - "Customized Lineplot with Log Scale"
...
Try replicating the plots above, customizing the legend, the axis text,
