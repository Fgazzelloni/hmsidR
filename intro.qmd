...
To be more specific, the book compares the metrics used to summarize the health status of a population across different locations. It tests prediction levels using key models. The initial tools are `{tidymodels}` and `{INLA}` for modeling, but other machine learning packages like `{mlr3}` and `{caret}` are also tested.
...
The purpose of this book is to contribute to the scientific development of the field of metrics and evaluation [@murray2008]. It is divided in four main sections, each containing three chapters. The first section **Metrics and Evaluation** is introductory explaining the health metrics definition at first level, the second section **Machine Learning** goes a bit more inside the topic looking at the tools available for the investigation, the third section **Data Visualization** equippe the user with the tools for being able to visualize the results of the analysis, and finally the fourth section makes the things going into practice providing **Case Studies**.

**First section** - This is the first section of the book and it aims at providing an overview of the **Metrics Evaluations**, the history of development and they usage along the time. In the first chapter **Health Metrics** there is a dip dive into the different type of metrics, the *Disability Adjusted Life Years (DALYs)* and the components: *Years of Life Lost (YLLs)* and *Years Lived with Disabilities (YLDs)*. Along with the definition of the metrics, in this chapter there is also a mention of a fourth metric, the *HALY* or the *Health Adjusted Life Years*, this is a further improvement made in the development of the health metrics and the evaluation of the state of health of a population.
...
The third chapter evaluates which are the **Causes and Risks** involved in influencing the level of the metrics, identifying the most dangerous enemies acting in favor of an increase in the number of years of life lost or lived with disabilities. Risks are considered stimulating alternative solutions for prevention and health policy development.

**Second section** - This is the the second level dip dive into the metrics evaluation, **Machine Learning**. A set of tools available for analyzing trends, or phenomenon, how to deal with missing values and an attempt in filling the missing data to provide an overall picture of the general trend is provided. In the first chapter **Techniques** there is an overview of the different types of models that would be suitable to use, the second chapter **Packages** provides a selection of the R-packages to use for modeling. Then chapter three shows how to make **Predictions**.

**Third section** - Focused on all about **Data Visualization**, the first chapter shows the **Application**, the second chapter applies **Spatials** modeling andvisualization to learn how to use these tools. Then, the third chapter further allows the reader to be able to improve the knowledge acquired with some tailored **Examples**.

**Fourth section** - The **Infectious Diseases** section contains two case studies, one is ***Covid19*** and the other one is **the case of *Malaria***. The third chapter of this last section is dedicated to a **Summary** the variation of the health metrics when these two infectious diseases affect the population.
