...
where $p_1$ and $p_0$ are the proportions of exposed and unexposed groups
...
favourable when exploring complex relationships among multiple variables.
...
(E) and different risk factors made with the `{ggdag}` package and
the `dagify()` function.
...
combination of risk factors\index{risk factors} need to be ascertained by
...
confirm the statement that *regular exercise leads to improved cardiovascular
