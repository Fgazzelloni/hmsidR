...
`{ggplot2}` package. In this chapter, we will discuss the history
and evolution of data visualisation, delve deeper into the topic,
...
visualisations, such as `{plotly}`, `{ggplotly}`, `{leaflet}`,
`{tmap}`, and `{shiny}`. These tools provide additional functionality
for creating interactive plots, maps, and dashboards, allowing for more
engaging and dynamic data visualisations.
...
Try replicating the plots above, customising the legend, the axis text,
and adding a log scale to the y-axis. You can also add a caption to the
plot, specifying the data source.
