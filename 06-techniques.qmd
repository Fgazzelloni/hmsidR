...
-   **Regression Models:** Regression models, including **linear
    ...
    **Lasso regression**\index{lasso regression}, are commonly used to
    ...
    appropriate techniques such as regularization.
...
-   **Predictive modelling:**\index{predictive modelling} Such as
    **decision trees**\index{decision trees}, **support vector machines
    ...
    high-risk populations, and optimize resource allocation.
...
learning digestible data. **Feature engineering**\index{feature engineering} is a technique that involves creating new features from existing ones based on domain knowledge or transformation of data to improve the model's ability to discern patterns. For example, creating features like **moving averages** or
...
modellings. In addition, tailored adjustments, and more sophisticated
...
analyzing its impact. Rabies, although nearly 100% fatal once symptoms
...
To address these challenges, we explore advanced modeling techniques
...
per 100,000 inhabitants due to rabies and all causes mortality in Asia
...
marginalized, poor and vulnerable populations. Although effective human
...
#|   - "Not scaled"
#|   - "Scaled and centered"
#| fig-alt: "DALYs due to Rabies - Not scaled and Scaled and centered"
#| echo: false
#| message: false
#| warning: false
...
p2 +
  labs(
    title = "Dalys and Deaths due to Rabies - Scaled and centered",
    subtitle = "in Asia and Global Region",
    color = "Location",
    y = "Value", x = "Time(Year)"
  ) +
...
#| fig-alt: "Deaths and DALYs due to Rabies and All Causes - Scaled and centered"
#| echo: false
#| message: false
#| warning: false
...
modelling specification, and evaluate the performance using resampling
...
evaluate the model's performance and estimate its generalization error.
...
choice is tipically done due to the algorithm's features, which is able
...
Generalized Linear Models (GLMs)\index{GLM} involve statistical
...
from the `glmnet`\index{glmnet} package, which fits a generalized linear
model via **penalized maximum likelihood**. The regularization path is
...
data.frame(dalys_rabies = response, predictors, predictions) %>%
  ggplot(aes(
    x = year, group = factor(locationGlobal),
...
3.  **K-Nearest Neighbors (KNN)**\index{KNN} models are a type of
