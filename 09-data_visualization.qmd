...
for a scatterplot, or `geom_line()` for a line plot, and so on. Then, with the `labs()` function, we can add titles, labels, and captions to the plot.
...
mapping of data to visual properties, such as color, fill, shape, and size,
making it easy to create a wide range of visualizations, or even new
data. In this example there is also the use of an alternative to the `labs()` function with the `ggtitle()` function, which differs in the way the title is placed in the plot.
...
Try replicating the plots above, customizing the legend, the axis text, and adding a log scale to the y-axis. You can also add a caption to the plot, specifying the data source.
