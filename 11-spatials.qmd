# Spatial data modeling and Visualization {#sec-11-spatials}
...
for-loop. We build a four vector dataframe with longitude, latitude,
...
maps made with `{ggplot2}`, `{ggmap}`, and `{laeflet}`.
...
I can apply Monte Carlo simulation by repeating the experiment, modeling
the distribution of transmission rates and infection outcomes. I will
repeat the scenario of infection spread (i.e., the distribution of
transmission rates and infection outcomes) 1,000 times. For each run, I
will calculate the resulting costs and losses associated with the spread
of infections, and I will store the results of each run in a results
dataframe.
