...
individuals exposed to a specific risk factor to the likelihood in those
who are not exposed. This metric helps quantify how much a risk factor,
like smoking or high blood pressure, increases the probability of an
...
combined effect of all risk exposures, which are categorized into
metabolic, behavioral, and environmental risk factors. Each category
...
and edges consider the potential interactions or dependencies between
different risk factors and outcomes. This approach is generally
favorable when exploring complex relationships among multiple variables.
...
By visualizing the relationships between these variables, we can
...
(E) and different risk factors is made with the `{ggdag}` package and
the `dagify()` function.
...
level is significantly influenced by behavioral, environmental, and
occupational risks. Behavioral risks, such as smoking and physical
...
variables, where one variable (*the cause*) directly influences or
determines the other (*the effect*). For example, regular exercise
improves cardiovascular health, or adequate sleep supports cognitive
function. However, causality is distinct from
correlation\index{correlation}, which indicates a statistical
association without implying a direct influence. For instance, a
correlation between television watching and obesity does not imply a
causal link. Establishing causality requires systematically evaluating
alternative explanations and accounting for confounding factors that
could affect both the cause and effect.
...
![Causal Inference
Flowchart](images/04_causal_inference_flowchart.png){#fig-causal_inference
fig-align="center" width="50%" fig-alt="Causal Inference Flowchart"}

Performing casual inference\index{causal inference} requires setting up
...
confirm the statement *regular exercise leads to improved cardiovascular
health*, an intervention may be designed to introduce another variable,
such as regular fruit consumption, and observe its combined effect with
...
cardiovascular health. Once the intervention is analyzed by measuring
